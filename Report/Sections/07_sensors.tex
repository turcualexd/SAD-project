...
The model implemented to simulate the behaviour of the sensor in the Simulink environment takes the real position of the S/C with respect to centre of Earth expressed in inertial space, changes its direction multiplying by -1 and expresses it in the $\mathcal{B}$ frame through the real attitude matrix $\boldsymbol{A_{BN}}$. 
...
\begin{itemize}[wide,itemsep=3pt,topsep=3pt]
    ...
    \item In the Simulink environment the white noise has to be defined through the noise power. This was selected as $N_p = \sigma^2 {T_s} $, 
...
Since magnetometer are usually characterized by low accuracy values, a deep search has been conducted to find a high-accuracy and low-noise typology. A suitable sensor is the \textit{MM200} furnished by \textit{AAC ClydeSpace}. The following performance parameters characterize the sensor:
...
Then the measurements errors have to be added, a first misalignment error modeled in the same way of the horizon sensor in \autoref{subsec:horizon_sensor}. For this case the perpendicular
